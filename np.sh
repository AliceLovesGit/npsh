...
E[2]="The script must be run as root, you can enter sudo -i and then download and run again. Feedback: [https://github.com/AliceLovesGit/npsh/issues]"
C[2]="必须以 root 方式运行脚本，可以输入 sudo -i 后重新下载运行，问题反馈:[https://github.com/AliceLovesGit/npsh/issues]"
E[3]="Unsupported architecture: \$(uname -m)"
C[3]="不支持的架构: \$(uname -m)"
E[4]="Please choose: "
C[4]="请选择: "
E[5]="The script supports Linux systems only. Feedback: [https://github.com/AliceLovesGit/npsh/issues]"
C[5]="本脚本只支持 Linux 系统，问题反馈:[https://github.com/AliceLovesGit/npsh/issues]"
...
E[84]="Please select the NodePass core to run. Use [np -t] to switch after installation:\\\n 1. Stable \$STABLE_LATEST_VERSION \(AliceLovesGit/nodepass\) - Suitable for production environments \(default\)\\\n 2. Development \$DEV_LATEST_VERSION \(AliceLovesGit/nodepass-core\) - Contains latest features, may be unstable\\\n 3. Classic \$LTS_LATEST_VERSION \(AliceLovesGit/nodepass-apt\) - Long-term support version"
C[84]="请选择要运行的 NodePass 内核，安装后可使用 [np -t] 切换:\\\n 1. 稳定版 \$STABLE_LATEST_VERSION \(AliceLovesGit/nodepass\) - 适合生产环境 \(默认\)\\\n 2. 开发版 \$DEV_LATEST_VERSION \(AliceLovesGit/nodepass-core\) - 包含最新功能，可能不稳定\\\n 3. 经典版 \$LTS_LATEST_VERSION \(AliceLovesGit/nodepass-apt\) - 长期支持版本"
...
check_cdn() {
  for PROXY_URL in "${GITHUB_PROXY[@]}"; do
    if [ "$DOWNLOAD_TOOL" = "curl" ]; then
      REMOTE_VERSION=$(curl -ksL --connect-timeout 3 --max-time 3 ${PROXY_URL}https://raw.githubusercontent.com/AliceLovesGit/npsh/refs/heads/main/README_EN.md 2>/dev/null | sed -nE 's/^-[ ]+(Stable|Development|LTS):/\1:/p')
    else
      REMOTE_VERSION=$(wget -qO- --no-check-certificate --tries=2 --timeout=3 ${PROXY_URL}https://raw.githubusercontent.com/AliceLovesGit/npsh/refs/heads/main/README_EN.md 2>/dev/null | sed -nE 's/^-[ ]+(Stable|Development|LTS):/\1:/p')
    fi
...
upgrade_nodepass() {
  ...
  if [ "$DOWNLOAD_TOOL" = "curl" ]; then
    ...
    if [ "$HAS_STABLE_UPGRADE" = 1 ]; then
      curl -sL "${GH_PROXY}https://github.com/AliceLovesGit/nodepass/releases/download/${STABLE_LATEST_VERSION}/nodepass_${STABLE_VERSION_NUM}_linux_${ARCH}.tar.gz" | tar -xz -C "$TEMP_DIR"
    ...
    # 下载稳定版
    if [ "$HAS_STABLE_UPGRADE" = 1 ]; then
      wget "${GH_PROXY}https://github.com/AliceLovesGit/nodepass/releases/download/${STABLE_LATEST_VERSION}/nodepass_${STABLE_VERSION_NUM}_linux_${ARCH}.tar.gz" -qO- | tar -xz -C "$TEMP_DIR"
      if [ -f "$TEMP_DIR/nodepass" ]; then
        mv "$TEMP_DIR/nodepass" "$WORK_DIR/np-stb"
        chmod +x "$WORK_DIR/np-stb"
      else
...
Documentation=https://github.com/AliceLovesGit/nodepass
...
bash <($DOWNLOAD_COMMAND https://raw.githubusercontent.com/AliceLovesGit/npsh/main/np.sh) \$1
EOF
...
│   >https://github.com/AliceLovesGit/nodepass   │
╰───────────────────────────────────────────╯ "
